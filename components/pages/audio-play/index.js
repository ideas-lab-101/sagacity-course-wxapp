  ...
  lifetimes: {
    created() {
      ...
      this.innerAudioContext = new AudioManager({
        onCanPlay: () => { this._audioManagerCanPlay() },
        onPlay: () => { this._audioManagerPlay() },
        onPause: () => { this._audioManagerPause() },
        onTimeUpdate: (params) => { this._audioManagerTimeUpdate(params) },
        onStop: () => { this._audioManagerStop() },
        onEnd: () => { this._audioManagerEnd() },
        onDestroy: () => { this._audioManagerDestroy() },
        onError: () => { this._audioManagerError(err) }
      })
    },
    attached() { },
    detached() {
